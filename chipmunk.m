...
if isfield(BpodSystem.ProtocolSettings,'labcamsAddress')
    ...
    try %could be assigned but if no camera available it will trow an error
...
%--------------------------------------------------------------------------
%This will be outsourced to the sma functions!
% %% Assign valves
...
%load(x,y) %loading from external calibration file
%
...
% Set the solenoid opening times for the respective reward volumes
% leftValveTime = GetValveTimes(S.leftRewardVolume, 1);
% rightValveTime = GetValveTimes(S.rightRewardVolume, 3);
% obsValveTime = GetValveTimes(S.obsRewardVolume, 4);
% 
% centerValveTime = 0; %This is not currently used in chipmunk
% %
%--------------------------------------------------------------------------
...
TrialSidesList = rand(1,maxTrialNum) > (1-S.propLeft); %Right side trials will be assigned 1 and left trials 0
%%TrialSidesList = TrialSidesList(randperm(maxTrials)); %Is this step really necessary? Random seed?
...
Rewarded = nan(1,maxTrialNum);
EarlyWithdrawal = nan(1,maxTrialNum);
DidNotChoose = nan(1,maxTrialNum);
ResponseSideRecord = nan(1,maxTrialNum);
ModalityRecord = nan(1,maxTrialNum);
CorrectSideRecord = nan(1,maxTrialNum);
RewardAmounts = nan(1,maxTrialNum);
...
% %     %Randomly select the stimulus modality for the current trial
% %     drawModality = rand; %Generate a ranom number between 0 and 1
% %     if drawModality < S.propOnlyVisual
% %     ModalityRecord(currentTrial) = 1; %Visual modality code is 1
% %     elseif drawModality < S.propOnlyVisual + S.propOnlyAuditory
% %         ModalityRecord(currentTrial) = 2; %Auditory modality code is 2
% %     else
% %     ModalityRecord(currentTrial) = 3; %Multi-sensory trials are identified by number 3
% %     end
%     
%% Check trial side and redraw according to user changes and biases
    ...
    if prevPropLeft ~= S.propLeft
        ...
        prevPropLeft = S.propLeft; %Retain this left side probability to compare the next time
            ...
            currentTrial,TrialSidesList,OutcomeRecord); %Update the display if changed
    ...
    elseif S.antiBiasStrength > 0 && prevPropLeft ~= S.propLeft
        ...
        if TrialsDone > 1 %Make sure to have some choice history
            if OutcomeRecord(TrialsDone) > -1 && OutcomeRecord(TrialsDone) < 2
                ...
                pLeft = getAntiBiasPleft(trialHistoryBiases, trialModalityBiases, ModalityRecord(currentTrial), ...
                    S.antiBiasStrength, ResponseSideRecord(TrialsDone), OutcomeRecord(TrialsDone));
                ...
                TrialSidesList(currentTrial) = rand < pLeft; %Changed from original more complicated: rand > (1-S.propLeft);
            end
        end
        outcomePlotLimits = OutcomePlotDemonstrator(BpodSystem.GUIHandles.OutcomePlotDemonstrator, 'refresh',...
            currentTrial,TrialSidesList,OutcomeRecord); %Update the display if changed
...
%
%--------------------------------------------------------------------------
%%Generate the stim train according to the chosen frequency
...
elseif ~BpodSystem.ProtocolSettings.isPoissonStim
  ...
  if stimulusRate < 9
    ...
    if ModalityRecord(currentTrial) == 3 %This is multi-sensory
        ...
        %always next the category boundary!
        auditoryIsiList = interStimulusIntervalList;
...
elseif ModalityRecord(currentTrial) == 2 %It's an auditory trial
    auditoryStimSignal = createAuditoryStimSignal(interStimulusIntervalList, SamplingFreq, stimulusDuration,...
        soundType, BpodSystem.ProtocolSettings.stimLoudness, addedNoiseLoudness, CalibrationModelParams);
...
% %         
% %         [stimEventList, actual_events, actual_duration] = get_possible_stim (M, stimulusRate);
% %         
% %         stimEventList;
...
%         
%         interStimulusIntervalList = set_Possion_stim(TotalStimDuration*1000, ToneDuration*1000, MinInterval*1000, stimulusRate);
%         
...
%             vis_isis = interStimulusIntervalList;
%             aud_isis = interStimulusIntervalList; % use the same intervals for both auditory and visual.
%             
%             
...
%                 
%                 vis_isis = interStimulusIntervalList;
%                 if interStimulusIntervalList(1) == 0    % if the first interval is 0ms, do not shuffle it.
%                     stimEventList_2 = interStimulusIntervalList(2:end);
%                     aud_isis = [0 stimEventList_2(randperm(length(stimEventList_2)))];
%                 else
%                     aud_isis = interStimulusIntervalList(randperm(length(interStimulusIntervalList)));
%                 end
%                 
%             else
%                 vis_isis = interStimulusIntervalList;
%                 aud_isis = set_Possion_stim(TotalStimDuration*1000, ToneDuration*1000, MinInterval*1000, stimulusRate);
...
%                 
%             else
%                 aud_isis = interStimulusIntervalList;
%                 a = 12;
...
% %     
% %     disp(['This event rate: ' num2str(stimulusRate) ' events/s'])
% %     
% %     
...
% %     
    ...
    if ~isempty(fieldnames(RawEvents))
        ...
        BpodSystem.Data.stimEventList{TrialsDone} = interStimulusIntervalList;
        ...
        BpodSystem.Data.EventRate(TrialsDone) = stimulusRate;
