...
maxTrials = 5000; %Constrain the number of trials but at rather high value
TrialSidesList = rand(1,maxTrials) > (1-S.propLeft); %Right side trials will be assigned 1 and left trials 0
...
choiceHistoryBiases = 0.5 * ones(2, 2, 2);
...
% display on the plot. Use a dummy set of parameters here instead of the
% real parameter set.
eval(['dummySma = ' S.experimentName 'SMA(''dummy'');']);
...
TrialsDone =  0; %Set this number to the trials actually finished
global update
update = 0; %Define the update variable

if BpodSystem.Status.BeingUsed %only run this code if protocol is still active
    disp('Set the camera and hit start')
    uiwait(BpodSystem.GUIHandles.Figures.DemonFigure); %wait for spout control and clear handle afterwards
else
