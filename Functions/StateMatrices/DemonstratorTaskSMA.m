...
if exist('correctSideOnCurrent') && ~isempty(correctSideOnCurrent) %input check
    ...
    if rewardedSide == 0 %the case for left side correct
        ...
        leftPortAction = 'DemonCorrectChoice';
    ...
    elseif rewardedSide == 1 %the case for right side correct
        ...
        rightPortAction = 'DemonCorrectChoice';
    end
...
if isOmmission %---> Play feedback sounds but do not deliver reward or punishment
    sma = AddState(sma,'Name', 'DemonCorrectChoice',...
    'Timer',0.2, ...
    'StateChangeConditions', {'Tup','FinishTrial'},...
    'OutputActions', {'SoftCode',255,'PWM2',255,'PWM4',255, 'SoftCode', 2});

    sma = AddState(sma, 'Name', 'DemonWrongChoice',...
    'Timer',0.2, ...
    'StateChangeConditions', {'Tup','FinishTrial'},...
    'OutputActions', {'SoftCode',255,'PWM2',255,'PWM4',255,'SoftCode',5});

else
    sma = AddState(sma,'Name', 'DemonCorrectChoice',...
    'Timer',0 , ...
    'StateChangeConditions', {'Tup','DemonReward'},...
    'OutputActions', {'SoftCode',255,'PWM2',255,'PWM4',255, 'SoftCode', 2});

    sma = AddState(sma,'Name', 'DemonReward',...
    'Timer',rewardValveTime, ...
    'StateChangeConditions', {'Tup','FinishTrial'},...
    'OutputActions', {'ValveState', RewardValve,incorrectPortLED, 255});
