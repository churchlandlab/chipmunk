function [sma, trialDelays, reviseChoiceFlag, pacedFlag] = FreelyMovingNonSocialObserverTaskSMA(correctSideOnCurrent);
% [sma, trialDelays, reviseChoiceFlag, pacedFlag] = NonSocialObserverTaskSMA(correctSideOnCurrent);
%
% SMA assembler function for the non-social version of the observer task
% where a virtual demonstrator completes the trials. In this experiment the
% actual outcomes and their sounds are delivered. In addition the LED on
% the rewarded poke is off if it is a valid, informative trial. This should
% tell the mouse which side was correct in the absence of a physical
% demonstrator. 
%
%
...
sma = AddState(sma, 'Name', 'Sync', ...
    ...
    'OutputActions',{'PWM1',255,'PWM2',255,'PWM3',255,'BNC2', 1});
...
%beam unbreakings. Explicitly deliver reward here to make the observer hear
%click of the valve.

sma = AddState(sma, 'Name', 'DemonWrongChoice',...
    'Timer',0, ...
    'StateChangeConditions', {'Tup','DemonWrongChoicePunishment'},...
    'OutputActions', {'PWM2',255, incorrectPortLED, 255,'SoftCode',255,'ObserverDeck1',31});
...
sma = AddState(sma, 'Name', 'DemonWrongChoicePunishment',...
    ...
    'StateChangeConditions', {'Tup','ObsCheckFixationSuccess'},...
    'OutputActions', {'PWM2',255, incorrectPortLED, 255, 'SoftCode', 5});
%Wrong choice punishment of the demonstrator leads to all LEDs except for the correct one being swithed on again
...
sma = AddState(sma, 'Name', 'DemonEarlyWithdrawalPunishment',...
    'Timer',outcomeSeparation, ...
    'StateChangeConditions', {'Tup','ObsCheckFixationSuccess'},...
    'OutputActions', {'PWM1',255,'PWM2',255,'PWM3',255, 'SoftCode', 4}); 
...
if ~isnan(earlyWithdrawalTime) %If this trial was completed by the virtual demonstrator keep the correct port unlit
    ...
    %registered and the trial will end. Note that at this point all the
    %LEDs are on only if it is an early withdrawal. Otherwise the correct
    %port LED will remain off (see below for this scenario)
    ...
    %demonstrator side on according to trial outcome. Give the observer the same amount of time to get to
    ...
    sma = AddState(sma, 'Name', 'ObsCheckFixationSuccess',...
        'Timer',0, ...
        'StateChangeConditions', {'ObserverDeck1_3','ObsWaitForRewardRetrieval','ObserverDeck1_4','ObsEarlyWithdrawal','ObserverDeck1_5','FinishTrial'},...
        'OutputActions', {'PWM2',255, incorrectPortLED, 255,'ObserverDeck1',32});
   %Same state as above but this time with the correct port unlit.
   
    sma = AddState(sma, 'Name', 'ObsWaitForRewardRetrieval',...
        'Timer',BpodSystem.ProtocolSettings.timeToChoose, ...
        'StateChangeConditions', {'Port4In','ObsReward', 'Tup','ObsDidNotHarvest'},...
        'OutputActions', {'PWM2',255, incorrectPortLED, 255});
    %Same again here. 
end
...
sma = AddState(sma, 'Name', 'ObsEarlyWithdrawal',...
    ...
    'StateChangeConditions', {'Tup','FinishTrial'},...
    'OutputActions', {'PWM1',255,'PWM2',255,'PWM3',255, 'SoftCode', 6});
